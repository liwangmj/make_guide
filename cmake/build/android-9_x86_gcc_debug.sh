...
cmake -DCPLATFORM=${CPLATFORM} -DCCPU=${CCPU} -DCCOMPILE=${CCOMPILE} -DCVER=${CVER} -DANDROID_ABI="${CCPU}" -DANDROID_NATIVE_API_LEVEL=${CPLATFORM} -DCMAKE_BUILD_TYPE=${CVER} -DCMAKE_TOOLCHAIN_FILE=../../${PROJECT_NAME}/toolchain/android.toolchain.cmake ../../${PROJECT_NAME}
#make -j8
cmake --build .
